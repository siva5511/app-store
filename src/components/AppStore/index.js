...
class AppStore extends Component {
  state = {activeTabId: 'SOCIAL', searchedFor: ''}

  render() {
    const {activeTabId, searchedFor} = this.state

    const onTabClick = tabId => {
      this.setState(prevState => ({
        activeTabId: tabId,
        searchedFor: prevState.searchedFor,
      }))
    }

    const onChangeOfIp = event => {
      this.setState(prevState => ({
        activeTabId: prevState.activeTabId,
        searchedFor: event.target.value,
      }))
    }

    const filteredApps = appsList
      .filter(applications => applications.category === activeTabId)
      .filter(app =>
        app.appName.toLowerCase().includes(searchedFor.toLowerCase()),
      )
    ...
    return (
      <div className="main-cont">
        <h1 className="header-prop">App Store</h1>
        <div className="search-ele-prop" placeholder="search">
          <input onChange={onChangeOfIp} className="ip-prop" type="search" />
          <img
            className="lens-prop"
            alt="search icon"
        ...
        <br />
        <ul className="category-name-container">
          {tabsList.map(each => (
            <TabItem
              key={each.tabId}
